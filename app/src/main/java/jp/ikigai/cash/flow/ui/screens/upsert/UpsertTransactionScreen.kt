...
import jp.ikigai.cash.flow.ui.components.bottomsheets.DatePickerLandscapeSheet
                ...
                Column(
                    ...
                    UpsertScreenBottomAppBar(
                        title = if (transactionId == 0L) {
                            stringResource(id = R.string.create_transaction_label)
                        } else {
                            stringResource(id = R.string.update_transaction_label)
                        },
                        enabled = enabled,
                        navigateBack = {
                        ...
                        deleteClick = if (transactionId > 0 && enabled) {
...
fun EntryProviderScope<NavKey>.upsertTransactionScreen(backStack: NavBackStack<NavKey>) {
    entry<UpsertTransactionRoute> { route ->
        val viewModel: UpsertTransactionScreenViewModel = koinViewModel {
            parametersOf(route.id, route.templateId)
        }
